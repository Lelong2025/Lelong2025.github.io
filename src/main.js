...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.addEventListener('mixing:open-auth', () => openAuth('login'))
...
async function submitAuth(event) {
  ...
  const result = mode === 'register'
    ? await auth.signUp(email, password, { display_name: displayName, full_name: displayName })
  ...
  if (mode === 'login' || result.session) setTimeout(closeAuth, 600)
}
