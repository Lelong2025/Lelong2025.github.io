...
// .wrangler/tmp/bundle-tpdDXr/checked-fetch.js
...
var resolveFetch = /* @__PURE__ */ __name((customFetch2) => {
  if (customFetch2) {
...
var resolveFetch2 = /* @__PURE__ */ __name((customFetch2) => {
...
var resolveFetch3 = /* @__PURE__ */ __name((customFetch2) => {
  if (customFetch2) {
    return (...args) => customFetch2(...args);
  }
  return (...args) => fetch(...args);
...
var resolveFetch4 = /* @__PURE__ */ __name((customFetch2) => {
  if (customFetch2) return (...args) => customFetch2(...args);
  return (...args) => fetch(...args);
}, "resolveFetch");
...
var fetchWithAuth = /* @__PURE__ */ __name((supabaseKey, supabaseUrl, getAccessToken, customFetch2, tracePropagationOptions) => {
...
// ../node_modules/@supabase/server/dist/verify-auth-RY8f0_YT.mjs
var EnvError = class extends Error {
  static {
    __name(this, "EnvError");
  }
  constructor(message2, code = EnvGenericError) {
    super(message2);
    this.status = 500;
    this.name = "EnvError";
    this.code = code;
...
var EnvGenericError = "ENV_ERROR";
var MissingSupabaseURLError = "MISSING_SUPABASE_URL";
var MissingPublishableKeyError = "MISSING_PUBLISHABLE_KEY";
var MissingDefaultPublishableKeyError = "MISSING_DEFAULT_PUBLISHABLE_KEY";
var MissingSecretKeyError = "MISSING_SECRET_KEY";
var MissingDefaultSecretKeyError = "MISSING_DEFAULT_SECRET_KEY";
var EnvErrorMap = {
...
var AuthError2 = class extends Error {
  ...
  constructor(message2, code = AuthGenericError, status = 401) {
    super(message2);
    this.name = "AuthError";
    this.code = code;
    this.status = status;
...
var AuthGenericError = "AUTH_ERROR";
var InvalidCredentialsError = "INVALID_CREDENTIALS";
var CreateSupabaseClientError = "CREATE_SUPABASE_CLIENT_ERROR";
var AuthErrorMap = {
  [InvalidCredentialsError]: () => new AuthError2("Invalid credentials", InvalidCredentialsError, 401),
  [CreateSupabaseClientError]: () => new AuthError2("Failed to create Supabase client", CreateSupabaseClientError, 500)
};
var Errors = {
  ...EnvErrorMap,
  ...AuthErrorMap
...
__name(parseJwksUrl, "parseJwksUrl");
...
__name(resolveJwks, "resolveJwks");
...
__name(createContextClient, "createContextClient");
...
var allowDeprecationWarned = false;
...
__name(warnAllowDeprecated, "warnAllowDeprecated");
...
__name(resolveAuthOption, "resolveAuthOption");
...
async function timingSafeEqual(a, b) {
  ...
  const [sigA, sigB] = await Promise.all([crypto.subtle.sign("HMAC", cryptoKey, encoder2.encode(a)), crypto.subtle.sign("HMAC", cryptoKey, encoder2.encode(b))]);
...
__name(parseAuthMode, "parseAuthMode");
...
__name(jwtClaimsToUserClaims, "jwtClaimsToUserClaims");
var INVALID = /* @__PURE__ */ Symbol("invalid");
var remoteJwksResolver = void 0;
function getJwksResolver(jwks) {
  ...
  __name(localJwtVerifyGetKey, "localJwtVerifyGetKey");
  return Object.assign(localJwtVerifyGetKey, { jwks: /* @__PURE__ */ __name(() => jwks, "jwks") });
}
__name(getJwksResolver, "getJwksResolver");
...
// ../node_modules/@supabase/server/dist/create-supabase-context-1ERhBQF_.mjs
async function createSupabaseContext(request, options) {
  ...
  try {
    ...
    return {
      data: null,
      error: e instanceof EnvError ? new AuthError2(e.message, e.code, 500) : Errors[CreateSupabaseClientError]()
    };
  }
}
__name(createSupabaseContext, "createSupabaseContext");
...
// ../node_modules/@supabase/server/dist/index.mjs
...
function addCorsHeaders(response, config) {
  ...
  const corsHeaders2 = buildCorsHeaders(config);
...
__name(withSupabase, "withSupabase");
...
var src_default = {
  async fetch(request, env, context) {
    ...
    const apiCors = buildCorsHeaders2(request, env);
...
async function handleChat(request, env, ctx) {
  const allowedOrigins = new Set(
    (env.ALLOWED_ORIGINS || "https://lelong2025.github.io,http://localhost:8787,http://127.0.0.1:5500").split(",").map((value) => value.trim()).filter(Boolean)
  );
  ...
  if (request.method === "OPTIONS") {
    return new Response(null, { status: originAllowed ? 204 : 403, headers: corsHeaders2 });
  }
  if (request.method === "GET") {
    return new Response("AI Journal Expert Worker is running \u{1F680}", {
      headers: { "Content-Type": "text/plain; charset=utf-8", ...corsHeaders2 }
    });
  }
  ...
  if (env.AI_RATE_LIMITER) {
    ...
    if (!success) {
      return new Response(JSON.stringify({ error: "Too many requests" }), {
        status: 429,
        headers: { "Content-Type": "application/json", "Retry-After": "60", ...corsHeaders2 }
      });
    }
  }
  try {
    ...
    const validMessages = Array.isArray(messages) && messages.length > 0 && messages.length <= 12 && messages.every((message2) => message2 && ["user", "assistant"].includes(message2.role) && typeof message2.content === "string" && message2.content.length > 0 && message2.content.length <= 2e3) && messages.reduce((sum, message2) => sum + message2.content.length, 0) <= 8e3;
    const validContext = contextData === void 0 || contextData && typeof contextData === "object" && JSON.stringify(contextData).length <= 25e3;
    ...
    if (!userId) {
      return jsonResponse({ error: "Authentication required" }, 401, corsHeaders2);
    }
    ...
    if (contextData) {
      ...
      if (hdgsnnMatches && hdgsnnMatches.length > 0) {
        contextText += "\n\n--- D\u1EEE LI\u1EC6U T\u1EA0P CH\xCD H\u0110GSNN (C\u1EE4C B\u1ED8) ---\n";
        hdgsnnMatches.forEach((item, idx) => {
          contextText += `${idx + 1}. T\xEAn t\u1EA1p ch\xED: ${item["T\xEAn T\u1EA1p ch\xED"] || "N/A"}
`;
          contextText += `   ISSN: ${item["ISSN"] || "N/A"}
`;
          contextText += `   Khung \u0111i\u1EC3m H\u0110GSNN: ${item["\u0110i\u1EC3m HDDGSNN"] !== void 0 ? item["\u0110i\u1EC3m HDDGSNN"] : "N/A"}
`;
        });
      }
...
`;
      ...
      if (scopusMatches && scopusMatches.length > 0) {
        ...
        scopusMatches.forEach((item, idx) => {
          contextText += `${idx + 1}. T\xEAn ngu\u1ED3n: ${item["Source Title"] || "N/A"}
`;
          contextText += `   ISSN: ${item["ISSN"] || "N/A"} | EISSN: ${item["EISSN"] || "N/A"}
`;
          contextText += `   Nh\xE0 xu\u1EA5t b\u1EA3n: ${item["Publisher"] || "N/A"}
`;
          contextText += `   Th\u1EDDi gian bao ph\u1EE7 (Coverage): ${item["Coverage"] || "N/A"}
`;
          contextText += `   Lo\u1EA1i ngu\u1ED3n: ${item["Source Type"] || "N/A"}
`;
          contextText += `   Tr\u1EA1ng th\xE1i ho\u1EA1t \u0111\u1ED9ng: ${item["Active or Inactive"] || "N/A"}
`;
          contextText += `   B\u1ECB Scopus ng\u1EEBng nh\u1EADn (Discontinued): ${item["Titles Discontinued by Scopus"] ? "C\xF3" : "Kh\xF4ng"}
`;
          contextText += `   Tr\u1EA1ng th\xE1i Open Access: ${item["Open Access Status"] || "Kh\xF4ng"}
`;
        });
      }
    }
    const systemPrompt = `B\u1EA1n l\xE0 m\u1ED9t chuy\xEAn gia h\xE0ng \u0111\u1EA7u v\u1EC1 c\xE1c t\u1EA1p ch\xED khoa h\u1ECDc trong v\xE0 ngo\xE0i n\u01B0\u1EDBc. B\u1EA1n h\u1ED7 tr\u1EE3 c\xE1c nh\xE0 nghi\xEAn c\u1EE9u tra c\u1EE9u v\xE0 t\xECm ki\u1EBFm th\xF4ng tin v\u1EC1 c\xE1c t\u1EA1p ch\xED khoa h\u1ECDc, bao g\u1ED3m \u0111i\u1EC3m H\u0110GSNN 2025 (H\u1ED9i \u0111\u1ED3ng Gi\xE1o s\u01B0 Nh\xE0 n\u01B0\u1EDBc Vi\u1EC7t Nam), ch\u1EC9 s\u1ED1 JCR (Impact Factor), ph\xE2n h\u1EA1ng Q (Quartile), v\xE0 tr\u1EA1ng th\xE1i Scopus.

H\xE3y tr\u1EA3 l\u1EDDi m\u1ED9t c\xE1ch l\u1ECBch s\u1EF1, chuy\xEAn nghi\u1EC7p, khoa h\u1ECDc, r\xF5 r\xE0ng v\xE0 trung th\u1EF1c b\u1EB1ng Ti\u1EBFng Vi\u1EC7t. \u0110\u1ECBnh d\u1EA1ng c\xE2u tr\u1EA3 l\u1EDDi c\u1EE7a b\u1EA1n th\u1EADt \u0111\u1EB9p m\u1EAFt b\u1EB1ng Markdown (s\u1EED d\u1EE5ng in \u0111\u1EADm, danh s\xE1ch g\u1EA1ch \u0111\u1EA7u d\xF2ng, b\u1EA3ng bi\u1EC3u, li\xEAn k\u1EBFt n\u1EBFu c\xF3).

D\u01B0\u1EDBi \u0111\xE2y l\xE0 k\u1EBFt qu\u1EA3 tra c\u1EE9u t\u1EEB c\u01A1 s\u1EDF d\u1EEF li\u1EC7u c\u1EE5c b\u1ED9 c\u1EE7a trang web (n\u1EBFu c\xF3):${contextText ? contextText : "\n(Kh\xF4ng c\xF3 d\u1EEF li\u1EC7u tr\xF9ng kh\u1EDBp trong c\u01A1 s\u1EDF d\u1EEF li\u1EC7u c\u1EE5c b\u1ED9)"}

H\u01AF\u1EDANG D\u1EAAN X\u1EEC L\xDD TH\xD4NG TIN:
1. N\u1EBFu c\xF3 D\u1EEE LI\u1EC6U C\u1EE4C B\u1ED8: H\xE3y \u01B0u ti\xEAn s\u1EED d\u1EE5ng th\xF4ng tin n\xE0y l\xE0m c\u0103n c\u1EE9 ch\xEDnh x\xE1c nh\u1EA5t \u0111\u1EC3 tr\u1EA3 l\u1EDDi chi ti\u1EBFt v\xE0 r\xF5 r\xE0ng v\u1EC1 t\u1EA1p ch\xED \u0111\xF3 (vd: \u0111i\u1EC3m s\u1ED1 c\u1EE5 th\u1EC3 n\u0103m 2025, IF th\u1EF1c t\u1EBF, tr\u1EA1ng th\xE1i Scopus). Tr\xE1nh suy \u0111o\xE1n kh\xE1c v\u1EDBi d\u1EEF li\u1EC7u \u0111\u01B0\u1EE3c cung c\u1EA5p.
2. N\u1EBFu KH\xD4NG C\xD3 D\u1EEE LI\u1EC6U C\u1EE4C B\u1ED8 ho\u1EB7c ng\u01B0\u1EDDi d\xF9ng h\u1ECFi c\xE2u h\u1ECFi chung chung b\xEAn ngo\xE0i: H\xE3y tr\u1EA3 l\u1EDDi d\u1EF1a tr\xEAn ki\u1EBFn th\u1EE9c s\xE2u r\u1ED9ng c\u1EE7a b\u1EA1n v\u1EC1 t\u1EA1p ch\xED khoa h\u1ECDc \u0111\xF3. N\u1EBFu b\u1EA1n kh\xF4ng ch\u1EAFc ch\u1EAFn v\u1EC1 m\u1ED9t th\xF4ng tin c\u1EE5 th\u1EC3 (nh\u01B0 \u0111i\u1EC3m H\u0110GSNN ch\xEDnh x\xE1c ho\u1EB7c IF m\u1EDBi nh\u1EA5t), h\xE3y n\xF3i r\xF5 v\xE0 khuy\xEAn ng\u01B0\u1EDDi d\xF9ng ki\u1EC3m tra b\u1EB1ng thanh t\xECm ki\u1EBFm ch\xEDnh c\u1EE7a trang web ho\u1EB7c v\u0103n b\u1EA3n ch\xEDnh th\u1EE9c c\u1EE7a H\u0110GSNN \u0111\u1EC3 \u0111\u1ED1i chi\u1EBFu.
3. Khi ng\u01B0\u1EDDi d\xF9ng h\u1ECFi v\u1EC1 m\u1ED9t t\u1EA1p ch\xED c\u1EE5 th\u1EC3, v\xED d\u1EE5 "T\u1EA1p ch\xED khoa h\u1ECDc l\u1EA1c h\u1ED3ng issn 2525-2186": H\xE3y tr\xEDch xu\u1EA5t th\xF4ng tin t\u1EEB d\u1EEF li\u1EC7u c\u1EE5c b\u1ED9 b\xEAn tr\xEAn (n\u1EBFu c\xF3) v\xE0 tr\xECnh b\xE0y th\u1EADt m\u1EA1ch l\u1EA1c: T\xEAn t\u1EA1p ch\xED, ch\u1EC9 s\u1ED1 ISSN, c\u01A1 quan/tr\u01B0\u1EDDng xu\u1EA5t b\u1EA3n, \u0111i\u1EC3m H\u0110GSNN ho\u1EB7c c\xE1c ch\u1EC9 s\u1ED1 qu\u1ED1c t\u1EBF t\u01B0\u01A1ng \u1EE9ng.
4. T\xCDCH H\u1EE2P T\xCCM KI\u1EBEM T\u1EA0P CH\xCD KHOA H\u1ECCC L\u1EA0C H\u1ED2NG (LHU Journal of Science):
   - Trang web ch\xEDnh th\u1EE9c l\xE0: https://tapchikhoahoc.lhu.edu.vn/
   - Khi ng\u01B0\u1EDDi d\xF9ng h\u1ECFi v\u1EC1 c\xE1c c\xF4ng tr\xECnh nghi\xEAn c\u1EE9u, b\xE0i b\xE1o khoa h\u1ECDc ho\u1EB7c t\u1EA1p ch\xED thu\u1ED9c \u0110\u1EA1i h\u1ECDc L\u1EA1c H\u1ED3ng, h\xE3y lu\xF4n h\u01B0\u1EDBng d\u1EABn h\u1ECD truy c\u1EADp ho\u1EB7c t\xECm ki\u1EBFm t\u1EA1i trang web n\xE0y.
   - \u0110\u1EB7c bi\u1EC7t, h\xE3y t\u1EF1 \u0111\u1ED9ng sinh m\u1ED9t li\xEAn k\u1EBFt t\xECm ki\u1EBFm tr\u1EF1c ti\u1EBFp cho h\u1ECD theo \u0111\u1ECBnh d\u1EA1ng: \`[T\xECm b\xE0i vi\u1EBFt tr\xEAn T\u1EA1p ch\xED Khoa h\u1ECDc L\u1EA1c H\u1ED3ng](https://tapchikhoahoc.lhu.edu.vn/search/?q=T\u1EEA_KH\xD3A_T\xCCM_KI\u1EBEM)\` (thay \`T\u1EEA_KH\xD3A_T\xCCM_KI\u1EBEM\` b\u1EB1ng t\u1EEB kh\xF3a t\xEAn b\xE0i b\xE1o, t\xEAn t\xE1c gi\u1EA3 ho\u1EB7c ch\u1EE7 \u0111\u1EC1 m\xE0 h\u1ECD \u0111ang h\u1ECFi \u0111\u1EC3 h\u1ECD ch\u1EC9 c\u1EA7n click l\xE0 tra c\u1EE9u \u0111\u01B0\u1EE3c ngay).`;
...
function buildCorsHeaders2(request, env) {
  const allowedOrigins = new Set(
    (env.ALLOWED_ORIGINS || "https://lelong2025.github.io,http://localhost:8787,http://127.0.0.1:5500").split(",").map((value) => value.trim()).filter(Boolean)
  );
  const origin = request.headers.get("Origin");
...
__name(buildCorsHeaders2, "buildCorsHeaders");
...
async function handleAccount(request, env, ctx, cors) {
  ...
  try {
    ...
    const [subRes, usageRes] = await Promise.all([
      ctx.supabase.from("subscriptions").select("status, expires_at, plan_id, vip_plans(name, price_vnd, duration_days, daily_ai_limit)").eq("user_id", userId).maybeSingle(),
      ctx.supabase.from("ai_usage").select("message_count").eq("user_id", userId).eq("usage_date", today).maybeSingle()
    ]);
    if (subRes.error) throw subRes.error;
    if (usageRes.error) throw usageRes.error;
    const subscription = subRes.data || null;
    const isVip = Boolean(subscription?.status === "active" && subscription.expires_at && new Date(subscription.expires_at) > /* @__PURE__ */ new Date());
    return jsonResponse({
...
__name(randomPaymentCode, "randomPaymentCode");
...
__name(orderPayload, "orderPayload");
async function handleCreateOrder(request, env, ctx, cors) {
  ...
  try {
    ...
    const { data: plans, error: planError } = await ctx.supabase.from("vip_plans").select("id, name, price_vnd, duration_days, payment_prefix").eq("id", "chatbox_ai").eq("active", true);
    if (planError) throw planError;
    const plan = plans?.[0];
    if (!plan) return jsonResponse({ error: "VIP plan is unavailable" }, 503, cors.headers);
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const { data: pending, error: pendingError } = await ctx.supabase.from("payments").select("id, payment_code, amount_vnd, status, expires_at").eq("user_id", userId).eq("status", "pending").gt("expires_at", now).order("created_at", { ascending: false }).limit(1);
    ...
    const { data: inserted, error: insertError } = await ctx.supabaseAdmin.from("payments").insert(payment).select("id, payment_code, amount_vnd, status, expires_at");
...
async function handleOrderStatus(request, env, ctx, cors, code) {
  ...
  try {
    const userId = ctx.userClaims?.id;
    if (!userId) return jsonResponse({ error: "Authentication required" }, 401, cors.headers);
    const { data: rows, error: statusError } = await ctx.supabase.from("payments").select("status, paid_at, expires_at").eq("user_id", userId).eq("payment_code", code).limit(1);
    if (statusError) throw statusError;
    if (!rows?.[0]) return jsonResponse({ error: "Order not found" }, 404, cors.headers);
    const row = rows[0];
    if (row.status === "pending" && new Date(row.expires_at) <= /* @__PURE__ */ new Date()) row.status = "expired";
...
async function verifySepaySignature(rawBody, request, secret) {
  ...
  if (!Number.isInteger(timestampNumber) || Math.abs(Date.now() / 1e3 - timestampNumber) > 300) return false;
...
__name(verifySepaySignature, "verifySepaySignature");
async function handleSepayWebhook(request, env) {
  ...
  try {
    ...
    if (!transactionId || !paymentCode || !Number.isSafeInteger(amount) || amount <= 0 || !incoming || expectedAccount && actualAccount !== expectedAccount) {
...
// C:/Users/Nhac Phuoc/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/Nhac Phuoc/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// .wrangler/tmp/bundle-tpdDXr/middleware-insertion-facade.js
...
// C:/Users/Nhac Phuoc/AppData/Local/npm-cache/_npx/32026684e21afda6/node_modules/wrangler/templates/middleware/common.ts
...
// .wrangler/tmp/bundle-tpdDXr/middleware-loader.entry.ts
